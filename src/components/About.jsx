  ...
  return (
    <section id="about" style={{ padding: '100px 0', background: 'rgba(255,255,255,0.02)' }}>
      <div className="container">
        <div ref={ref} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'center' }}>
          <div style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateX(0)' : 'translateX(-40px)', transition: 'all 0.8s ease' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              {values.map((v, i) => (
                <div key={i} className="card-hover" style={{ background: 'rgba(108,99,255,0.08)', border: '1px solid rgba(108,99,255,0.2)', borderRadius: '12px', padding: '16px', display: 'flex', alignItems: 'center', gap: '12px', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(20px)', transition: `all 0.5s ease ${0.3 + i * 0.1}s` }}>
          ...
          <div style={{ position: 'relative', opacity: visible ? 1 : 0, transform: visible ? 'translateX(0)' : 'translateX(40px)', transition: 'all 0.8s ease 0.2s' }}>
            <img src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=600&h=400&fit=crop" alt="Team"
              style={{ width: '100%', borderRadius: '20px', objectFit: 'cover', boxShadow: '0 40px 80px rgba(0,0,0,0.5)' }} />
            <div className="animate-pulse-glow" style={{ position: 'absolute', bottom: '-20px', left: '20px', background: 'linear-gradient(135deg, #6C63FF, #00D4FF)', borderRadius: '16px', padding: '20px 28px' }}>
